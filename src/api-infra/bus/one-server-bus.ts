...
import { AnonymousUser, AuthUser, ModuleCaller } from '#api/controller/types.js';
...
export class OneServerBus implements Bus {
  ...
  async publish(eventPublish: PublishBusMessage): Promise<void> {
    ...
    if (!events) return;
    const event = events.find((e) => e.publishNessageName === eventPublish.name);
    if (!event) return;
    ...
    const caller: ModuleCaller = {
      ...
      user: eventDod.caller as AuthUser | AnonymousUser,
    };
